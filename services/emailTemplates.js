          ...
          <table role="presentation" width="100%" style="max-width:520px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #E7DFD2;">
            <tr>
              <td style="background:${INK};padding:20px 28px;">
                <span style="color:#ffffff;font-size:18px;letter-spacing:0.04em;">RelateReady</span>
              </td>
            </tr>
            <tr>
...
function pendingReportReminderEmail({ name, lang, resultsUrl }) {
  ...
  if (lang === "en") {
    return {
      ...
      html: shell({
        ...
        bodyHtml: `
          <p style="font-size:16px;margin:0 0 16px;">Hi ${firstName},</p>
          <p style="font-size:15px;line-height:1.6;margin:0 0 16px;">A little while ago you completed the RelateReady relationship-readiness test — your 8-pillar snapshot is ready and waiting for you.</p>
  ...
  return {
    ...
    html: shell({
      ...
      bodyHtml: `
        <p style="font-size:16px;margin:0 0 16px;">Hola ${firstName},</p>
        <p style="font-size:15px;line-height:1.6;margin:0 0 16px;">Hace un tiempo completaste el test de preparación relacional de RelateReady — tu radiografía de los 8 pilares ya está lista y te espera.</p>
